...
/**
 A two button `UIView` that manages a passwordless experience with Beyond Identity.
...
public class AuthView: View {
    ...
    let signInButton = SignInButton(title: LocalizedString.signInButtonTitle.string)
    let signUpButton = SignUpButton(title: LocalizedString.signUpButtonTitle.string)
    
    /// Initalize an `AuthView`
    /// - Parameters:
    ///   - url: A URL with the http or https scheme pointing to the authentication webpage.
    ///   - callbackURLScheme: The custom URL scheme that the app expects in the callback URL.
    ///   - completionHandler: A completion handler the session calls when it completes successfully, or when the user cancels the session.
    ...
    private func setUpViews() {
        ...
        let stackView = StackView(arrangedSubviews: [signInButton, signUpButton])
...
extension AuthView: ASWebAuthenticationPresentationContextProviding {
    /// Conforming to `ASWebAuthenticationPresentationContextProviding`, this tells the delegate it should present content to the user from its `superview.window`.
