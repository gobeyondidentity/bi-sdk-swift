...
var urlComps = URLComponents(string: "https://auth.byndid.run/v2/authorize")!
urlComps.queryItems = [
...
let session = ASWebAuthenticationSession(
    ...
    completionHandler: { url, error in
        if let error = error { print(error) }
        if let url = url { print(url) }
        guard let currentState = url.queryParameters?["state"], currentState == self.state! else {
            print("Incorrect state")
            return
        }

        if let code = url.queryParameters?["code"] else {
